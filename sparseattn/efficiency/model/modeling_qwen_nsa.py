...
from sparseattn.training.block_sparse_attention_triton.native_sparse_attention.module import qwen3_nsa
...
class Qwen3NSA_prefill(nn.Module):
    def __init__(self, config: Qwen3Config, layer_idx: int):
        ...
        self.sliding_window = (
            ...
            if getattr(config, 'layer_types', [None]*100)[layer_idx] == "sliding_attention" 
            else None
        )
        #===========================
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        query_states = self.q_norm(self.q_proj(hidden_states).view(hidden_shape)).transpose(1, 2)#[batch, q_head, seqlen, head_dim]
        key_states = self.k_norm(self.k_proj(hidden_states).view(hidden_shape)).transpose(1, 2)#[batch, k_head, seqlen, head_dim]
        value_states = self.v_proj(hidden_states).view(hidden_shape).transpose(1, 2)#[batch, k_head, seqlen, head_dim]
        ...
        if is_decode:
            ...
            # 无论是decode阶段还是在prefill阶段，这边的压缩都是一样的
            ...
            topk_idx = topk_idx.transpose(0,1).contiguous()#[num_k_heads,valid_seqlen_q, num_attend_idx]
...
class Qwen3DecoderLayer(nn.Module):
    def __init__(self, config: Qwen3Config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = Qwen3NSA_prefill(config=config, layer_idx=layer_idx)
...
if __name__ == "__main__":
    ...
    saved_model_path = "/data2/hf_models/Qwen3-4B"
