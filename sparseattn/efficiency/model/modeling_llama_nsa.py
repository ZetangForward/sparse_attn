...
from sparseattn.training.block_sparse_attention_triton.native_sparse_attention.module import llama_nsa
...
from sparseattn.training.block_sparse_attention_triton.ops import (
...
class LlamaNSA_prefill(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        self.init_blocks = config.init_blocks
        self.local_blocks = config.local_blocks
        self.window_size = config.window_size

        # nsa compress func
        self.compress_func = COMPRESS_TYPE_TO_FUNC[config.compress_type]

        # nsa parameteres
        self.compress_key = COMPRESS_TYPE_TO_WEIGHT[config.compress_type](
            self.num_key_value_heads, self.head_dim, self.kernel_size
        )

        self.compress_value = COMPRESS_TYPE_TO_WEIGHT[config.compress_type](
            self.num_key_value_heads, self.head_dim, self.kernel_size
        )

        # gate function
        self.gate = torch.nn.Sequential(
            torch.nn.Linear(self.hidden_size, self.num_heads * 3, bias=False),
            torch.nn.Sigmoid(),
        )        
        
        # init parameters
        self.init_params()#如果不初始化，gate,compress_key,compress_value中的权重是0
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        input_shape = hidden_states.shape[:-1]#[batch, seqlen, hidden_size]
        hidden_shape = (*input_shape, -1, self.head_dim)#[batch, seqlen, heads, head_dim]

        query_states = self.q_proj(hidden_states).view(hidden_shape).transpose(1, 2).contiguous() #[batch, q_head, seqlen, head_dim]
        key_states = self.k_proj(hidden_states).view(hidden_shape).transpose(1, 2).contiguous() #[batch, k_head, seqlen, head_dim]
        value_states = self.v_proj(hidden_states).view(hidden_shape).transpose(1, 2).contiguous() #[batch, k_head, seqlen, head_dim]
        ...
        if attention_mask is None:
            attention_mask = torch.full((batch_size, key_states.shape[-2]), True, dtype=torch.bool, device=hidden_states.device)
        ...
        if is_decode:
            ...
            q, _, cu_seqlens_q, _ = unpad_input(query_states.transpose(1,2), attention_mask)
            k, indices, cu_seqlens_k, _ = unpad_input(key_states.transpose(1,2), attention_mask)#[total_kv_len, num_kv_heads, head_dim]
            v, _, _, _ = unpad_input(value_states.transpose(1,2), attention_mask)
            q = q.contiguous()
            k = k.contiguous()
            v = v.contiguous()
            ...
            compressed_k, compressed_cu_seqlens = linear_compress(
                ...
                self.kernel_stride,
                None,
            )#[compress_kv_len, num_kv_heads, head_dim]
            ...
            max_seqlen_q = (cu_seqlens_q[1:] - cu_seqlens_q[:-1]).max().item() #这表示当前batch中最大的有效的tokens长度
            ...
            # print(topk_idx.shape)#[num_kv_heads, total_q_len, topk]
            # print(topk_idx[:,0,:])
            head_mask_type = torch.ones(k.shape[1], dtype=q.dtype, device=q.device)

            topk_idx = topk_idx.transpose(0, 1).contiguous()#[valid_seqlen_q, num_k_heads,num_attend_idx]
            with torch.cuda.device(q.device):
                sparse_attn_output, _ = block_sparse_attention(
                    q,k,v,
                    topk_idx,self.block_size,self.block_size,cu_seqlens_q,cu_seqlens_k,
                    max_seqlen_q,max_seqlen_k,self.scaling,self.is_causal,head_mask_type
                )
            ...
            # gate average
            gate = self.gate(unpadded_hidden)#[total_len, hidden_size] -> [total_len, num_q_heads * 3]
            gate = rearrange(gate, "n (h g) -> n h g", g=3)#[total_len, num_q_heads, 3]

            #[total_len, num_q_heads] * [total_len, num_q_heads, head_dim]
            #相当于对于每一个token的每个head的head_dim都要乘以一个gate权重
            attn_output = (
            ...
            attn_output = pad_input(attn_output, indices, batch_size, seqlen_q).contiguous()#[batch_size, seqlen_q_len, num_q_heads, head_dim]

            attn_output = attn_output.reshape(batch_size, seqlen_q, -1).contiguous()#[batch_size, max_seqlen_q, num_q_heads * head_dim]
            attn_output = self.o_proj(attn_output).contiguous()#[batch_size, max_seqlen_q, hidden_size]
            attn_weights = None
            
        return attn_output, attn_weights, spa
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        self.self_attn = LlamaNSA_prefill(config=config, layer_idx=layer_idx)
...
class BaseModelOutputWithPast(ModelOutput):
    ...
    model_sparsity: Optional[Tuple[torch.FloatTensor, ...]] = None
...
if __name__ == "__main__":
    ...
    saved_model_path = "/data2/hf_models/Meta-Llama-3.1-8B-Instruct"
