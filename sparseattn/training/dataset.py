...
class DataCollator:
    ...
    def __call__(self, features):
        ...
        for item in features:
            ...
            indices = (
                item["indices"] if "indices" in item else [(0, len(item["input_ids"]))]
            )
            ...
            for a, b in indices:
                ...
                if b - a > 1:
                    ...
                    assert available_tokens == 0, (
                        "Available tokens should be non-negative"
                    )
                    break
...
def build_dataset(
    paths, training_args: TrainingArguments, data_args: DataArguments, is_training: bool
) -> StreamingDataset:
    ...
    for path in paths:
        if "@" in path:
            ...
            streams.append(
                SafeStream(remote=path, local=path, proportion=float(proportion))
            )
        elif "#" in path:
            path, proportion = path.split("#", 1)
            logger.info(f"Loading dataset from {path} with repeat {proportion}")
            streams.append(
                SafeStream(remote=path, local=path, repeat=float(proportion))
            )
        else:
    ...
    assert per_device_step_size % num_dataloaders == 0, (
        "dataloader workers should divide local batch size"
    )
