...
class AttentionRouter(nn.Module):
    ...
    def forward(
        ...
        fname = f"router_states_layer{layer_idx}_{unique_id}.pt"
        save_path = os.path.join("/data1/lcm_lab/qqt/SparseAttn/tests/pooled_latent", fname)
        # task_ids = 0
        task_ids = torch.tensor([0] * bsz)
        payload = {
            "pooled_hidden_states": pooled_latent.detach().cpu(), # [B, H, D]
            "task_ids": task_ids.detach().cpu(), # [B]
            "layer_id": layer_idx
        }
        try:
...
class Qwen3Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
            ...
            if not self.config.enable_ada_sparsity:
                ...
                if unpadded_lengths is not None:
                    res = self.mask_allocator(
                        k, unpadded_lengths[0], range_ids, task_ids, layer_idx=self.layer_idx
                    )
                else:
                    res = self.mask_allocator(k, None, range_ids, task_ids, layer_idx=self.layer_idx)
        ...
        if not has_layer_past:
            import os
            output_dir = "/data1/lcm_lab/qqt/SparseAttn/examples" 
            os.makedirs(output_dir, exist_ok=True)

            file_path = os.path.join(output_dir, "qwen3-8b_output_log_per_task.txt")

            output_data = f"head allocate: {[x.tolist() for x in z_kv_batch]}"
...
class Qwen3Model(Qwen3PreTrainedModel):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                Qwen3DecoderLayer(config, context_window_toggle=context_window_toggle, layer_idx=i)
