...
class infllmv2_LlamaConfig(LlamaConfig):
    model_type = "infllmv2_llama"  # 建议设置自定义 model_type，便于识别

    def __init__(self, sparse_config=None, **kwargs):
        kwargs["attn_implementation"] = "flash_attention_2"
        kwargs["architectures"] = ["infllmv2_LlamaForCausalLM"]
...
class infllmv2_LlamaAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(self, config: infllmv2_LlamaConfig, layer_idx: int):
        super().__init__()
        self.config = config
...
class infllmv2_LlamaDecoderLayer(nn.Module):
    def __init__(self, config: infllmv2_LlamaConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = infllmv2_LlamaAttention(config=config, layer_idx=layer_idx)
...
class infllmv2_LlamaPreTrainedModel(PreTrainedModel):
    config_class = infllmv2_LlamaConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["infllmv2_LlamaDecoderLayer"]
...
class infllmv2_LlamaModel(infllmv2_LlamaPreTrainedModel):
    ...
    def __init__(self, config: infllmv2_LlamaConfig):
        ...
        self.layers = nn.ModuleList(
            [
                infllmv2_LlamaDecoderLayer(config, layer_idx)
...
class infllmv2_LlamaForCausalLM(infllmv2_LlamaPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = infllmv2_LlamaModel(config)
        self.vocab_size = config.vocab_size
...
__all__ = [
    "infllmv2_LlamaForCausalLM",
    "infllmv2_LlamaModel",
    "infllmv2_LlamaPreTrainedModel",
]


if __name__ == "__main__":
    config_path = "/data2/hf_models/Meta-Llama-3.1-8B-Instruct/"
    config = infllmv2_LlamaConfig.from_pretrained(config_path)
    print(config)
