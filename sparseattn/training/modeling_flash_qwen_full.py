...
# ----------------------------------------------------------------------------
# 核心修改：Qwen3Attention (变为 Dense Baseline)
# ----------------------------------------------------------------------------


class Qwen3Attention(nn.Module):
    """
    Modified for Baseline: Pure Dense Flash Attention.
    Removes AttentionRouter, Sparsity Masks, and Streaming logic.
    """

    def __init__(
        self, config: PawQwen3Config, context_window_toggle: Optional[int] = 1024
    ):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
        ...
        # --- 移除所有 Router, Mask, Streaming 相关的初始化 ---
        # 仅保留为了兼容性可能需要的空属性（如果外部有强依赖，否则直接删除）
    ...
    def forward(
        ...
        **kwargs,
        ...
        # 2. Context Parallel Handling (Gather Global QKV)
        is_cp_enabled = (
        ...
        if is_cp_enabled:
            ...
            # 此时 q, k, v shape: [S_global, H_local, D]
        ...
        if position_ids is not None:
            # 注意：如果启用了 CP，unpadded_lengths 应该是全局的长度，Rotary 需要正确处理
            pass  # Rotary logic handled inside
        ...
        if has_layer_past:
            ...
            if new_len > past_kv.size(1):
                # Resize cache logic (simplified from original)
                past_kv = torch.cat(
                    [
                        past_kv,
                        torch.empty(
        ...
        present_key_value = (past_kv, past_len + q.size(1)) if use_cache else None

        # 5. Standard Flash Attention (Dense)
        # --- 这里的逻辑被完全替换为标准的 FlashAttn 调用，去掉了 mask/router ---

        # 准备 Flash Attn 输入
        # 如果是 CP 模式，q 已经是 [S_global, H, D]
        ...
        if unpadded_lengths is not None:
            ...
            # Standard Batch Flash Attention
        ...
        # 6. Context Parallel Output Handling
        if is_cp_enabled:
            # Reverse SeqAllToAll: [S_global, H_local, D] -> [S_local, H_total, D]
        ...
        # 8. Construct Dummy Outputs for Compatibility
        # 原返回: z_sum, entropy, pooled_hidden_states, z_constrast, attn_output, attn_weights, past_key_value

        # Dummy values (全是 None 或 0，保持类型兼容)
        dummy_z_sum = torch.tensor(
            0.0, device=hidden_states.device, dtype=hidden_states.dtype
        )
        dummy_entropy = torch.tensor(
            0.0, device=hidden_states.device, dtype=hidden_states.dtype
        )
        dummy_pooled = None
        dummy_z_contrast = torch.tensor(
            0.0, device=hidden_states.device, dtype=hidden_states.dtype
        )  # 或者 None，视外层 loss 计算而定
        attn_weights = None
...
class Qwen3DecoderLayer(nn.Module):
    def __init__(
        self, config: PawQwen3Config, context_window_toggle: Optional[int] = 4096
    ):
        super().__init__()
        self.hidden_size = config.hidden_size
    ...
    # 移除或保留 mask 相关函数为空实现，防止调用报错
    def set_threshold_for_deterministic(self, val):
        pass

    def get_masks(self):
        return None

    def reset_masks(self, val=4.0):
        pass

    def fill_masks_with_value(self, val):
        pass
...
# ----------------------------------------------------------------------------
# Model 修改 (移除 Sparsity 计算)
# ----------------------------------------------------------------------------


class Qwen3Model(Qwen3PreTrainedModel):
    def __init__(self, config: PawQwen3Config):
        ...
        # --- 移除 Sparsity Parameters ---
        # 原来的 lambda 参数不再需要初始化，防止显存占用，或者保留为 None
    ...
    def forward(
        ...
        **flash_attn_kwargs,
    ) -> Union[Tuple, BaseModelOutputWithPastAndSparsity]:
        # ... 标准的参数处理 ...
        output_attentions = (
            output_attentions
        ...
        # ... Gradient Checkpointing 处理 (同原代码) ...
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = torch.utils.checkpoint.checkpoint(
                    ...
                    attention_mask,
                    position_ids,
                    None,  # past_key_value is None in CP
                    unpadded_lengths,
                    output_attentions,
                    False,  # use_cache
                    seq_parallel_group,
            ...
            # 解包：忽略 dummy 的 z_sum 等
            # (z_sum, entropy, pooled_hidden_states, z_constrast, hidden_states, ...)
            hidden_states = layer_outputs[4]
        ...
        # --- 移除所有 Sparsity 聚合与 Loss 计算 ---
        # 强制将所有 Sparsity 相关的字段设为 None 或 0
        model_sparsity = torch.tensor(0.0, device=hidden_states.device)
        target_sparsity_out = torch.tensor(0.0, device=hidden_states.device)
        z_loss = torch.tensor(0.0, device=hidden_states.device)
        head_entropy = torch.tensor(0.0, device=hidden_states.device)
        ...
        return BaseModelOutputWithPastAndSparsity(
            ...
            target_sparsity=target_sparsity_out,
            sparsity_loss=z_loss,
            lambda1=None,
            lambda2=None,
            layerwise_model_sparsity=None,
            layerwise_target_sparsity=None,
            log_z_loss=None,
...
# ----------------------------------------------------------------------------
# ForCausalLM 修改 (主要是适配 Model 的输出)
# ----------------------------------------------------------------------------
