...
class MixedAttention(torch.autograd.Function):
    ...
    def backward(ctx, d_output, *args):
        ...
        dmk = torch.zeros_like(
            moba_kv[:, 0], dtype=moba_kv.dtype, device=moba_kv.device
        )
        dmv = torch.zeros_like(
            moba_kv[:, 1], dtype=moba_kv.dtype, device=moba_kv.device
        )
        _flash_attn_varlen_backward(
            dout=d_moba_output,
            q=moba_q,
...
def moba_attn_varlen(
    ...
    moba_q_indices = moba_q_indices.nonzero(as_tuple=True)[
        -1
    ]  # [HS indices] * N (total size: all non-zero elements in HS dimension)
