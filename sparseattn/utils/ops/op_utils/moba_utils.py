...
def shuffle_input_all(
    ...
    seq_offsets = torch.Tensor([seq_offset_val, seq_offset_val + block_seq_len]).to(
        to_send.device
    )
...
def compute_moba_gate(
    ...
    if len(q.shape) == 4:
        q, k, v = tensor_4d_to_3d(q), tensor_4d_to_3d(k), tensor_4d_to_3d(v)
    ...
    kv_list = [
        torch.zeros_like(kv, dtype=q.dtype, device=q.device) for _ in range(world_size)
    ]
    ...
    gate_seq_idx = torch.arange(
        seq_offset,
        min(seq_offset + block_size, seqlen),
        device=q.device,
        dtype=torch.int32,
    )[None, :].repeat(num_filtered_chunk, 1)
