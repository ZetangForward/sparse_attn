...
def Xattention_prefill_dim4(
    ...
    for i in range(batch_size):
        ...
        # ================= NEW: 计算 Last Token Sparsity =================
        # 1. 取出最后一个 Query Block 对应的 Mask 行
        # approx_mask shape: [1, num_heads, q_blocks, k_blocks]
        # 我们取第 valid_q_blocks - 1 行，且只看有效的 k_blocks 列
        last_block_mask = approx_mask[0, :, valid_q_blocks - 1, :valid_k_blocks] # shape: [num_heads, valid_k_blocks]
        
        # 2. 统计所有头中，最后一个 Block 关注的 Key Block 总数
        total_active_blocks_last_token = last_block_mask.sum().item()
        
        # 3. 计算平均每个头关注的 Block 数
        avg_active_blocks = total_active_blocks_last_token / num_heads
        
        # 4. 估算 attend_tokens (近似值：块数 * 块大小)
        attend_tokens = avg_active_blocks * block_size
        
        # 5. 计算稀疏度
        # 公式: 1 - (attend_tokens / all_tokens)
        # all_tokens 即当前的序列长度 valid_len
        if valid_len > 0:
            last_token_sparsity = 1.0 - (attend_tokens / valid_len)
        else:
            last_token_sparsity = 0.0
            
        # print(f"[Batch {i}] Len: {valid_len}, Attend: {int(attend_tokens)}, Sparsity (Last Token): {last_token_sparsity:.4f}")
        # ================================================================
    ...
    return attn_output, last_token_sparsity
