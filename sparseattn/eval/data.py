...
def load_qa(dataset, path, demo_path, max_test_samples=None, popularity_threshold=None, shots=0):
    ...
    if len(data) > max_test_samples:
        data = datasets.Dataset.from_dict(data[:max_test_samples])
...
def load_icl(dataset, max_test_sample=None, seed=42):
    ...
    def load_trec_data(filepath):
        ...
        with open(filepath, "rb") as f:
            for line in f:
                line = line.replace(b"\xf0", b" ").strip()  # 处理非法字节
                if not line:
                    continue
                try:
                    decoded_line = line.decode("utf-8")
                except UnicodeDecodeError as e:
                    print(f"Decode error: {line[:50]}... -> Error: {e}")
                    continue
                ...
                if len(parts) < 2:
                    continue
                label, text = parts
                fine_label = label.strip()
                coarse_label = label.split(":")[0] if ":" in label else label

                data["text"].append(text)
                data["fine_label"].append(fine_label)
                data["coarse_label"].append(coarse_label)

        coarse_label_names = sorted(set(data["coarse_label"]))
        fine_label_names = sorted(set(data["fine_label"]))
    ...
    if "trec_fine" in dataset.lower():
        ...
        train_data = load_dataset("./data/banking77", trust_remote_code=True)["train"]
        test_data = load_dataset("./data/banking77", trust_remote_code=True)["test"]
