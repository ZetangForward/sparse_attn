...
def locret_kv_eviction_for_cache(
    ...
    for layer in range(n_layers):
        ...
        if (
            target_key_device is not None
            and attention_tensor.device != target_key_device
        ):
        ...
        if scores[layer] is not None:
            ...
            if (
                target_key_device is not None
                and scores[layer].device != target_key_device
            ):
                scores[layer] = scores[layer].to(target_key_device)

            scores[layer] = torch.cat(
                (scores[layer], attention_tensor[: end_index - start_index]), dim=-2
            )
        else:
        ...
        if end_index < input_len - local_len:
            sc[:, -stabilizers:, :] = torch.finfo(
                sc.dtype
            ).max  # always keep `stabilizers` last kvs if this is not the last chunk
        indices = torch.topk(sc[0, :, :], k=n_selected, dim=-2).indices

        indices = (
            indices.transpose(0, 1).sort().values
        ...
        scores[layer] = torch.gather(
            ...
            indices.transpose(0, 1).unsqueeze(
                0
            ),  # back to the original shape after sorting
        )

        # Create indices tensors for key and value separately (in case they have different shapes)
        key_indices = (
            indices.unsqueeze(0).unsqueeze(-1).repeat(kv_shape[0], 1, 1, kv_shape[3])
        )
        value_indices = (
            indices.unsqueeze(0).unsqueeze(-1).repeat(kv_shape[0], 1, 1, kv_shape[3])
        )
        ...
        if (
            target_value_device is not None
            and value_indices.device != target_value_device
        ):
