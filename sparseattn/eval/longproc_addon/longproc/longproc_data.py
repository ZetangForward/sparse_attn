...
def _load_html_to_tsv_data(
    dataset_name: str, path: str = None
) -> Tuple[Dict, Callable]:
...
def eval_html_to_tsv(prediction: str, example: dict):
    ...
    eval_results = evaluate_html_to_csv_compute_metrics(
        prediction, example["reference_output"]
    )
    return {
        "f1": eval_results["f1"],
        "precision": eval_results["precision"],
        "recall": eval_results["recall"],
        "extraction_rate": 1.0 if eval_results["error"] is None else 0.0,
    }, {"parsed_output": prediction, "error_msg": eval_results["error"]}
...
def eval_countdown(prediction: str, example: dict):
    ...
    if pred_solution is not None and evaluate_countdown_final_solution(
        nums, target, pred_solution
    ):
        return {"accuracy": 1.0, "partial_accuracy": 1.0, "extraction_rate": 1.0}, {
            "parsed_output": pred_solution,
        }
    ...
    gt_procedure = (
        ground_truth_procedure.split("# Search Procedure")[-1]
...
def _load_countdown_data(dataset_name: str, path: str = None) -> Tuple[Dict, Callable]:
    ...
    def build_icl_demonstration():
        ...
        for demo in _DEMO_SET:
            _, demonstration = build_countdown_demonstration(
                demo["nums"], demo["target"]
            )
            examples.append(
                f"# Example\nNumbers: {demo['nums']}\nTarget: {demo['target']}\n\n{demonstration}"
            )
    ...
    user_prompt = user_prompt.format(
        demonstration=build_icl_demonstration(), nums="{nums}", target="{target}"
    )
    ...
    for d in data:
        ...
        solution_str = "\n".join(solution)
        assert evaluate_countdown_final_solution(nums, target, solution_str), (
            f"Failed to evaluate solution {solution_str}"
        )
        data_purged.append(
            {
                "nums": nums,
...
def eval_travel_planning(prediction: str, example: dict):
    ...
    if plan_text is not None:
        accuracy = evaluate_travel_plan_solution(
            data_item["ground_truth_cities"],
            data_item["ground_truth_durations"],
            plan_text,
        )
    else:
        accuracy = 0.0
    ...
    if accuracy == 1.0:
        ...
        partial_accuracy, error_report = evaluate_travel_plan_search_procedure(
            data_item, prediction, ground_truth_procedure
        )
    return {
        "accuracy": accuracy,
        "partial_accuracy": partial_accuracy,
...
def _load_travel_planning_data(
    dataset_name: str, path: str = None
) -> Tuple[Dict, Callable]:
    ...
    data = [
        d
        for d in data
        if output_range[0] <= d["estimated_output_tokens"] < output_range[1]
    ]
    with open(os.path.join(path, "prompts.yaml"), "r") as f:
        prompt = yaml.safe_load(f)
        user_prompt = prompt["USER_PROMPT"]
    ...
    # partially fill the user prompt
    user_prompt = user_prompt.format(
        demonstration=build_icl_demonstration(), problem="{problem}"
    )
...
def load_longproc_data(dataset_name: str, path: str = None) -> Tuple[List, Callable]:
    ...
    assert all(
        [
            "input_prompt" in d and "reference_output" in d and "item" in d
            for d in upacked_data
        ]
    )
