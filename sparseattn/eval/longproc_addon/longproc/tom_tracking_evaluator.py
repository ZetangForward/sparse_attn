...
def evaluate_tom_trace(model_response_text, ground_truth_text):
    ...
    model_beliefs = [
        ...
        for line in model_responses
        if _extract_belief_content(line)
    ]
    ground_truth_beliefs = [
        _extract_belief_content(line)
        for line in ground_truth_responses
        if _extract_belief_content(line)
    ]
    ...
    if len(model_beliefs) == len(ground_truth_beliefs) and all(
        a == b for a, b in zip(model_beliefs, ground_truth_beliefs)
    ):
        strict_accuracy = 1.0
        ...
        first_diff = next(
            (
                i
                for i, (a, b) in enumerate(zip(model_beliefs, ground_truth_beliefs))
                if a != b
            ),
            None,
        )
        if first_diff is not None:
            ...
            partial_accuracy = min(len(model_beliefs), len(ground_truth_beliefs)) / len(
                ground_truth_beliefs
            )
