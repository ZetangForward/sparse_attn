...
def load_all_logs(path):
    ...
    if os.path.isdir(path):
        ...
        try:
            ...
            raise RuntimeError(
                f"❌ Failed to load single file {path}. Error: {e}"
            ) from e
...
def analyze_router_inputs(
    log_file_path, max_samples=2000, avg_heads=True, perplexity=30
):
    print("\n==============================================")
    ...
    for chunk in data_chunks:
        ...
        if avg_heads:
            ...
            tasks_data = np.repeat(
                [ID_TO_TASK.get(tid, "Unknown") for tid in t_chunk], H
            )
    ...
    print(
        f"Final samples used for analysis (N): {num_samples} (Max limit: {max_samples})"
    )
    ...
    if tsne_perplexity < 1:
        ...
        if num_samples < 15:
            print(
                f"⚠️ Warning: Sample size is too small ({num_samples}). Setting perplexity to {tsne_perplexity}. Results may be unreliable."
            )
    ...
    try:
        tsne = TSNE(
            n_components=2, perplexity=tsne_perplexity, n_iter=1000, random_state=42
        )
        X_embedded = tsne.fit_transform(X)
    except ValueError as e:
        print(f"❌ t-SNE failed. Detail: {e}")
        print(
            f"HINT: Check if you have enough unique data points ({num_samples}) for perplexity={tsne_perplexity}."
        )
        return

    df_tsne = pd.DataFrame(
        {"x_dim1": X_embedded[:, 0], "x_dim2": X_embedded[:, 1], "Task": y}
    )
    ...
    plt.subplot(1, 2, 1)
    sns.scatterplot(
        data=df_tsne, x="x_dim1", y="x_dim2", hue="Task", palette="bright", alpha=0.7
    )
    ...
    if len(valid_tasks) > 1:
        ...
        sns.heatmap(
            ...
            fmt=".2f",
            cmap="coolwarm",
            xticklabels=valid_tasks,
            yticklabels=valid_tasks,
            vmin=0,
            vmax=1,
        )
        ...
        if max_sim > 0.95:
            print(
                "❌ CRITICAL: Inputs are extremely similar. The 'first_token' is not differentiating tasks effectively."
            )
        elif max_sim > 0.8:
            print(
                "⚠️ WARNING: Tasks are close. Requires strong MLP/high logit separation."
            )
        else:
            print(
                "✅ PASS: Inputs are well separated. The foundation for routing is good."
            )

    else:
        print("\n=== Diagnosis: Similarity Matrix ===")
        print(
            "Insufficient task diversity (less than 2 unique tasks) to calculate similarity matrix."
        )
