...
def stats(t, name):
    print(
        f"{name}: mean={t.mean():.3f}, std={t.std():.3f}, min={t.min():.3f}, max={t.max():.3f}"
    )
...
print(
    f"z1_soft vs z2_soft L1 diff (per element): {torch.abs(z1_soft - z2_soft).mean():.5f}"
)
...
for h in range(H):
    print(
        f"  Head {h}: z1={z1_head_mean[0, h]:.3f}, z2={z2_head_mean[0, h]:.3f}, diff={abs(z1_head_mean[0, h] - z2_head_mean[0, h]):.3f}"
    )
