...
regex_rank = re.compile(
    r"^Rank \d+: \[Step (\d+)] Task=\['(.+?)'\] \| model_sparsity=tensor\(\[([\d\.]+)"
)
...
regex_info = re.compile(
    r"^\[INFO\|.+\] .* >> \{'lm_loss': ([\d\.]+), 'reg_loss': ([\d\.]+), 'loss': ([\d\.]+), 'target_sparsity': ([\d\.]+), 'model_sparsity': ([\d\.]+), 'step': (\d+)"
)
...
def calculate_target_sparsity(
    steps: np.ndarray, start: float, end: float, total_steps: int, warmup_ratio: float
) -> np.ndarray:
    """计算目标稀疏度随 Step 变化的曲线"""
...
def plot_all_tasks_with_global_target(
    df_agg: pd.DataFrame, config: Dict[str, Dict[str, float]]
):
    ...
    for task in tasks:
        ...
        plt.plot(
            task_data["step"],
            task_data["model_sparsity"],
            marker=".",
            linestyle="-",
            alpha=0.8,
            label=f"{task} Model Sparsity",
        )
...
def plot_individual_tasks(df_agg: pd.DataFrame, config: Dict[str, Dict[str, float]]):
    ...
    for i, task in enumerate(tasks):
        ...
        current_config = config.get(
            task, config.get("default", {"start": 0.0, "end": 0.7})
        )
        ...
        task_target_curve = calculate_target_sparsity(
            task_steps_range, start, end, TOTAL_STEPS, WARMUP_RATIO
        )
    ...
    fig.suptitle(
        "Individual Task Model Sparsity vs. Target Sparsity", fontsize=18, y=1.02
    )
...
if __name__ == "__main__":
    ...
    if aggregated_data is not None and not aggregated_data.empty:
        ...
        print(
            "未从日志中提取到 Model Sparsity 数据，可能是文件内容为空或正则表达式需要调整。"
        )
