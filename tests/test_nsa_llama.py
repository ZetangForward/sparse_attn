...
def print_model_summary(model):
    ...
    for name, module in model.named_modules():
        ...
        if (
            param_count > 0 or buffer_count > 0
        ):  # Only print if the module has parameters or buffers
            print(f"Layer: {name}")
...
def compare_models_weights(current_model_path, original_model_path, tolerance=1e-5):
    ...
    for i in range(len(current_model.model.layers)):
        ...
        gate_match = torch.allclose(
            current_mlp.gate_proj.weight, original_mlp.gate_proj.weight, atol=tolerance
        )
        up_match = torch.allclose(
            current_mlp.up_proj.weight, original_mlp.up_proj.weight, atol=tolerance
        )
        down_match = torch.allclose(
            current_mlp.down_proj.weight, original_mlp.down_proj.weight, atol=tolerance
        )
    ...
    for i in range(len(current_model.model.layers)):
        ...
        ln1_weight_match = torch.allclose(
            current_ln1.weight, original_ln1.weight, atol=tolerance
        )
        ln2_weight_match = torch.allclose(
            current_ln2.weight, original_ln2.weight, atol=tolerance
        )
    ...
    lm_head_match = torch.allclose(
        current_model.lm_head.weight, original_model.lm_head.weight, atol=tolerance
    )
    ...
    for i in range(len(current_model.model.layers)):
        ...
        if hasattr(current_attn, "q_proj") and hasattr(original_attn, "q_proj"):
            q_match = torch.allclose(
                current_attn.q_proj.weight, original_attn.q_proj.weight, atol=tolerance
            )
        else:
            print(
                f"Warning: 'q_proj' not found in layer {i} attention modules. Skipping Q projection comparison."
            )
        ...
        if hasattr(current_attn, "k_proj") and hasattr(original_attn, "k_proj"):
            k_match = torch.allclose(
                current_attn.k_proj.weight, original_attn.k_proj.weight, atol=tolerance
            )
        else:
            print(
                f"Warning: 'k_proj' not found in layer {i} attention modules. Skipping K projection comparison."
            )
            k_match = True

        if hasattr(current_attn, "v_proj") and hasattr(original_attn, "v_proj"):
            v_match = torch.allclose(
                current_attn.v_proj.weight, original_attn.v_proj.weight, atol=tolerance
            )
        else:
            print(
                f"Warning: 'v_proj' not found in layer {i} attention modules. Skipping V projection comparison."
            )
            v_match = True

        if hasattr(current_attn, "o_proj") and hasattr(original_attn, "o_proj"):
            o_match = torch.allclose(
                current_attn.o_proj.weight, original_attn.o_proj.weight, atol=tolerance
            )
        else:
            print(
                f"Warning: 'o_proj' not found in layer {i} attention modules. Skipping O projection comparison."
            )
            o_match = True

        if not (q_match and k_match and v_match and o_match):
            print(
                f"Attention projection weights in layer {i} do NOT match (or attributes missing). This is expected if LlamaNSA redefines projections."
            )
            attn_proj_match = False
            break  # Stop at first mismatch
    if attn_proj_match:
        print(
            "All Attention projection weights match across all layers (or attributes not found)."
        )
...
def load_and_verify_model(saved_model_path, original_model_path):
    ...
    print(
        f"Model config architectures: {model.config.architectures}"
    )  # 应该打印 ['NSALlamaForCausalLM']
