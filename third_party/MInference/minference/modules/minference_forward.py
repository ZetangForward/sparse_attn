...
def gather_last_q_vertical_slash_topk_v4(self, q, k, v, head_id):
    ...
    def vertical_and_slash_kernel_extend(q, k, v, vertical_size, slash_size):
        vertical_size, slash_size = (
            min(q_len, max(vertical_size + 100, 30)),
            min(q_len, max(slash_size, 50)),
        )
        last_q = min(64, q_len)
        last_start = 100
...
def gather_last_q_vertical_slash_topk_v4_sparsity(self, q, k, v, head_id):
    ...
    if self.config.to_dict().get("dilated1", False) or self.config.to_dict().get(
        "dilated2", False
    ):
        return dilated_sparsity()
    if self.config.to_dict().get("dense", False):
        return dense_sparsity()
    if self.config.to_dict().get("a_shape", False) or self.config.to_dict().get(
        "tri_shape", False
    ):
        return vertical_slash_sparsity(
            self.config.streaming_kwargs["n_init"],
            self.config.streaming_kwargs["n_local"],
        )
    if self.config.to_dict().get("static_pattern", False) or self.config.to_dict().get(
        "vs_only", False
    ):
        ty, vertical_size, slash_size, _ = self.best_pattern.get(
            head_id, ("vertical_and_slash", 1000, 6096, 1)
